...
passwordFile = open('dbPassword.txt', 'r')
...
[psg.Text("Album Image Link:"), psg.Input(key='-IMGURLB-', justification='left', size=(40, 40)),
 psg.Text("Track Number:"), psg.Input(key='-TRACKNUMB-', justification='left', size=(10, 10))],
[psg.T("")],
...
def insertSubmission(mp3_hash, artistName, songName, albumName, yearReleased, genre, trackNum, albumImageLink, userSub):
    rejectErr = 0
    if not str(yearReleased).isnumeric() or not str(trackNum).isnumeric():
    ...
    if rejectErr == 0:
        ...
        if not albumDupes:
            ...
            dbCursor.execute(
                ...
                (newAlbumID, albumName, 0, albumImageLink))
        else:
            dbCursor.execute("SELECT * FROM ALBUMARTISTDUPECHECK WHERE ARTIST_NAME=%s AND ALBUM_TITLE=%s",
                             (artistName, albumName))
        ...
        if not songDupes:
            ...
            dbCursor.execute("INSERT INTO genres(GENRE, ALBUM_ID, SONG_TITLE) VALUES (%s, %s, %s)",
                             (genre, newAlbumID, songName))
            ...
            dbCursor.execute(
                "INSERT INTO submission(SUBMISSION_ID, TIME_OF_SUBMISSION, MP3_HASH, ARTIST_NAME, SONG_TITLE, ALBUM_ID, ALBUM_TITLE, YEAR_RELEASED, GENRE, TRACK_NUM, IMAGE_LINK) VALUES (%s, NOW(), %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                (newSubmissionID, mp3_hash, artistName, songName, newAlbumID, albumName, yearReleased, genre, trackNum,
                 albumImageLink))
...
def deleteSubmission(prevSubData):
    ...
    dbCursor.execute("DELETE FROM genres WHERE ALBUM_ID = %s AND GENRE=%s AND SONG_TITLE=%s",
                     (newAlbumID, prevSubData[8], prevSubData[4]))
...
while True:
    ...
    ##runs if the submit data button is pressed, after the user has entered the information
        ...
        rejectErr = insertSubmission(mp3_hash, artistName, songName, albumName, yearReleased, genre, trackNum, albumImageLink, True)
        ...
        if rejectErr == 0:
            ...
            updateList = ['-SONGB-', '-ARTISTB-', '-ALBUMB-', '-YEARB-', '-GENREB-', '-TRACKNUMB-', '-IMGURLB-', '-ERRMESSAGEB-', '-ERRMESSAGE-', '-JSONINPUT-']
            for i in updateList:
                window[i].update("")
    ...
    ##Appends the information displayed to the tags of the MP3 that was submitted
        ...
        updateList = ['-SONGB-', '-ARTISTB-', '-ALBUMB-', '-YEARB-', '-GENREB-', '-TRACKNUMB-', '-IMGURLB-', '-ERRMESSAGEB-', '-JSONINPUT-']
        for i in updateList:
            window[i].update("")
        ...
        for JSONfile in JSONlist:
            ...
            if rejectErr == 0:
                ...
                if duplicates:
                    ...
                    rejectErr = insertSubmission(mp3_hash, artistName, songName, albumName, yearReleased, genre, trackNum, albumImageLink, False)
                    if rejectErr == 0:
    ...
    ##sends the user from the information gathering screen to the main screen when the back button is pressed. Must be different from previous back button as two elements cannot have the same key, which is used to determine the event
        ...
        window[f'-COL2-'].update(visible=False)
        window[f'-COL3-'].update(visible=False)
        updateList = ['-SONGB-', '-ARTISTB-', '-ALBUMB-', '-YEARB-', '-GENREB-', '-TRACKNUMB-', '-IMGURLB-', '-ERRMESSAGEB-', '-ERRMESSAGE-', '-FILEINPUT-', '-JSONINPUT-']
        for i in updateList:
            window[i].update("")
